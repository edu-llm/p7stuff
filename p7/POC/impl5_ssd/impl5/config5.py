...
SAMPLING_DEFAULT = "T1"
MAX_NEW_TOKENS = 128                   # ~p99 of gold turn length at ~1.35 tok/word

# PLAN §3.2. Appended to the *content of the last user message* rather than inserted as a
# new turn, so role alternation is identical to training and the divergence from the
# training prefix is a checkable suffix (see chat5.assert_reference_suffix_only).
REFERENCE_TEMPLATE = (
...
def reference_block(gold: str) -> str:
    return REFERENCE_JOIN + REFERENCE_TEMPLATE.format(gold=gold)
